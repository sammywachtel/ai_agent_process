...
- **Explicit `<project_root>/.agent_process/work/{scope}/.run/...` paths everywhere** — Swept ~125 ambiguous `.run/...` references across orchestration coordinators, step files, and process docs, replacing each with the fully-qualified explicit form (`<project_root>/.agent_process/work/{scope}/.run/...`, `<project_root>/.agent_process/work/{scope_or_noscope}/.run/...` for release files, or `<project_root>/.agent_process/brainstorms/{chosen_name}/.run/...` for brainstorm files). Closes the recurring failure mode where sub-agents wrote `.run/` artifacts to the project root or to whatever cwd they happened to land in, because the spawned-agent prompt only said "Output: .run/foo.md" without a folder context the sub-agent could resolve. Coordinators establish the convention at the top, but sub-agents only see the prompt text — every reference in the prompt is now self-contained.
- **PR shepherd: bounded single-pass contract (sub-agent compatible)** — Reframed `process/pr-shepherd.md` and `orchestration/steps/release/095-pr-shepherd.md` from "monitor until merge-ready" (a long-lived watcher contract incompatible with sub-agent execution) to "do one bounded pass and report." The sub-agent now snapshots CI/review state, attempts simple in-diff fixes (one attempt per issue per pass), and classifies the outcome as MERGE-READY / NEEDS-FOLLOWUP / BLOCKED. Ongoing monitoring is the responsibility of `/loop` or `/schedule` re-invoking the pass at intervals — the shepherd itself never sleeps, polls, or waits across the bounded execution window. Closes the failure mode where the shepherd sub-agent complained that "long-running watch isn't supported for a sub-agent" because the contract told it to do something the runtime can't deliver.
- **Human prereqs: filter at planning time + structured human-readable form** — Two-part rewrite of how `human-prereqs.md` is generated and presented. Closes the failure mode where the planning step surfaces orchestrator-decidable technical questions ("should we exclude feature X because the existing storage contract doesn't support it?") to the human in working-vocabulary form ("Step 3 locked criteria intentionally excluded it because the X table and the /api/v1/Y contracts do not support Z today"), confounding developers who have no anchor for what the question even means.

  *Part 1 — Filter at planning time (`orchestration/coordinators/plan-scope.md`).* The default answer to "should this be a human prereq?" is now NO. For every candidate item the planner must answer YES to all three: (1) Can the orchestrator structurally NOT make this decision? (external credentials, deploy approvals, contract changes that cross teams, requirement-named features being deferred — these PASS; "should we use approach A or B given equally-supported patterns" — that FAILS, the planner decides); (2) Could a developer who doesn't know the codebase's internal vocabulary answer this in 30 seconds without a glossary? (3) What concrete user-visible consequence changes based on the answer? Items that fail any question fold into the iteration plan's **Design Decisions** table (new explicit subsection — see below) where the orchestrator records the call and the reviewer can override at review time.

  *Part 2 — Structured human-readable form for items that pass the filter.* New required structure for every prereq item: plain-language question (no codebase jargon), "What this means" (1–2 sentences in user-outcome terms), "Recommended answer" (orchestrator's recommendation with one-line reason), "If you say YES" / "If you say NO" (user-outcome consequences), "Override required because" (which of the four PASS reasons applies — the receipt that the filter was applied; if the planner can't fill this in, the item shouldn't be there). Two worked examples included in the coordinator: one item that fails the filter and goes into Design Decisions, one that passes and uses the full structure.

  *Execute coordinator (`orchestration/coordinators/execute.md`)* updated to present the structured form to the human, and to translate legacy/loose items into user-outcome language at surfacing time — strip working vocabulary (internal step numbers, internal table or contract identifiers, planning artifacts) before the human sees them. If translation isn't possible, the executor flags the item explicitly rather than dumping raw planning text on the user.

  *Iteration plan template (`templates/iteration-plan.md`)* gains an explicit **Design Decisions (made by orchestrator, not human prereqs)** subsection inside Technical Assessment — the documented home for choices the planner made that didn't need human input. The reviewer reads this and may flag any choice they disagree with at review time; the human is not paused for these.
- **§1.5 broadened: alternatives, state, and concrete behavior scenarios** — Extends `steps/execution/02-prepare.md` §1.5 from "universal quantifier / 'and' enumeration" to four triggers: (1) universal quantifiers, (2) "and" enumeration (existing), (3) **"or" alternatives with divergent semantics** — fires when an AC names alternatives that may have different shapes (e.g., auth via API key / OAuth token / session cookie have divergent lifetime and revocation rules; HTTP GET / HEAD / OPTIONS have divergent body and idempotency semantics); reading the alternatives as symmetric is the failure mode, (4) **state-dependent behavior** — fires when an AC describes behavior that varies by ambient state (list populated vs empty, auth state, cache hit/miss, rate-limit window position, write in flight vs persisted, cold vs warm). Tightens what counts as a §1.5 *cell*: every row in the matrix must be a **concrete behavior scenario** with literal input (request body / URL / event / user action), named state context, and literal observable outcome (HTTP response / log line / queue message / UI state / downstream side effect) — code-surface labels (e.g., "layer A vs layer B," "service A vs service B," "compose vs read surface") supplement scenarios but cannot replace them. Worked example added showing the trigger × input × state matrix for a backend auth API (login / logout with rate-limit + lock states) — applies symmetrically to MCP tools, message-queue handlers, and frontend UI ACs. Closes the failure mode where the §1.5 matrix proved code paths exist while leaving real outcomes unproven (state-cliff cases like "valid creds against locked account" or "fallback path when the gating list is empty").
