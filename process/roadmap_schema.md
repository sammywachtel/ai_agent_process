...
├── master_roadmap.md           # Consolidated roadmap (all status, categories, requirements)
├── backlog.md                  # Prioritized work queue (future items)
├── .roadmap_config.json        # User configuration and project mappings
├── archived_roadmap.md         # (created on first archive) Archived requirements
└── .roadmap_audit.jsonl        # (created on first status change) Audit trail
```
...
- **Link related items** (requirements ↔ work scopes)
